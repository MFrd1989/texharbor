...
function Workspace({ user }: { user: UserDto }) {
  ...
  const jumpFromPdf = async (location: SyncTexLocationDto) => { const file = files.find((item) => item.id === location.fileId); if (!file) { setError(`Source file ${location.path} is not available.`); return; } if (selected?.id !== file.id) await openFile(file); setSourceJumpRequest({ line: location.line, column: location.column, nonce: Date.now() }); };
